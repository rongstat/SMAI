...
########################################
load("global.RData")
metadata=read.csv(file="global_metadata.csv")
data.X1.ct = data.X1.c
data.X2.ct = data.X2.c
ct0.X1 = ct.X1
ct0.X2 = ct.X2
...
data.X1.c = data.X1.ct[,sel.id]
ct.X1 = ct0.X1[sel.id]

sel.id = sample(22090,n.size[i])
data.X2.c = data.X2.ct[,sel.id]
ct.X2 = ct0.X2[sel.id]

ct.X1=factor(metadata$Majority_voting_CellTypist_high[match(colnames(data.X1.c),metadata$X)])
ct.X2=factor(metadata$Majority_voting_CellTypist_high[match(colnames(data.X2.c),metadata$X)])
table(ct.X1)
table(ct.X2)

ct.X1=factor(metadata$Manually_curated_celltype[match(colnames(data.X1.c),metadata$X)])
ct.X2=factor(metadata$Manually_curated_celltype[match(colnames(data.X2.c),metadata$X)])
...
prop.align.par#if smaller than 0.3, set prop.align=prop.align.par instead.
...
#repeat chunk above for various k
data.plot = data.frame(similarity=c(sim.gene.det.mat), method=rep(c("SMAI", "fastMNN", "Scanorama","Seurat","LIGER"),each=20))
data.plot = data.plot[-(1:20),]
pdf(file = paste0("markers_boxplot_Neg3.pdf"), width=3,height=3)
...
data.plot = data.frame(similarity=c(sim.gene.det.mat), method=rep(c("SMAI", "fastMNN", "Scanorama","Seurat","LIGER"),each=20))
data.plot = data.plot[-(1:20),]
pdf(file = paste0("FD_boxplot_Neg3.pdf"), width=3,height=3)
...
sim.gene.det.mat = rbind(sim.gene.det.mat, sim.gene.det)
#repeat chunk above
rownames(sim.gene.det.mat)=levels(ct.X2)



data.plot = data.frame(similarity=c(sim.gene.det.mat), method=rep(c("SMAI", "fastMNN", "Scanorama","Seurat","LIGER"),each=20))
data.plot = data.plot[-(1:20),]
pdf(file = paste0("power_boxplot_Neg3.pdf"), width=3,height=3)
...
pdf(file = paste0("Neg3_dist.pdf"), width=3,height=3)
