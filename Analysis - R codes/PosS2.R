...
RNA = read.table('PosS2/scRNA_count.txt')
spatial = read.table('PosS2/Spatial_count.txt', header = T)
...
for(i in gene.sel){
  ...
  set.seed(20)
  out.smai <- align(data.X1, data.X2, denoise = "scree", dir.map = "2to1", r.max=100, outlier.cut=1, cutoff = 1.025,
  ...
  knnmodel = knnreg(t(data2), RNA.sub[,i])
  pred_y = predict(knnmodel, data.frame(t(data1)))
  out1[i,1] = cor(pred_y, spatial[match(colnames(data1), rownames(spatial)),i])
  out1[i,2] = cor(pred_y, spatial[match(colnames(data1), rownames(spatial)),i], method="kendall")
  out1[i,3] = cor(pred_y, spatial[match(colnames(data1), rownames(spatial)),i], method="spearman")
...
save(out1,out2,out3,out4,out5,out6,file="spatial13.RData")
load("spatial13.RData")
...
pdf(file = "task10_kend_top50_cor.pdf", width=3,height=3)
p
dev.off()

########alignment test

mnn.out = findMutualNN(t(data1),t(data2), k1=40)
set.seed(3)
out.smai <- align(data.X1, data.X2, denoise = "scree", dir.map = "2to1", r.max=100, outlier.cut=1, cutoff = 1.025,
                  sel1=unique(mnn.out$first),  sel2=unique(mnn.out$second),
                  prop.align=0.3, t=3)
