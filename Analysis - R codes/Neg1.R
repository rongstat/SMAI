...
#select cell types
data.X3 = cbind(data.X1[,which(ct.X1=="acinar")], data.X1[,which(ct.X1=="activated_stellate")],
                data.X1[,which(ct.X1=="alpha")], data.X1[,which(ct.X1=="endothelial")])
ct.X3=ct.X1[c(which(ct.X1=="acinar"),which(ct.X1=="activated_stellate"),which(ct.X1=="alpha"),which(ct.X1=="endothelial"))]
data.X4 = cbind(data.X2[,which(ct.X2=="beta")], data.X2[,which(ct.X2=="delta")],
                data.X2[,which(ct.X2=="ductal")], data.X2[,which(ct.X2=="gamma")])
ct.X4=ct.X2[c(which(ct.X2=="beta"),which(ct.X2=="delta"),which(ct.X2=="ductal"),which(ct.X2=="gamma"))]

#data.X3 = cbind(data.X1[,which(ct.X1=="acinar")], data.X1[,which(ct.X1=="delta")],
#                data.X1[,which(ct.X1=="alpha")], data.X1[,which(ct.X1=="gamma")])
#ct.X3=ct.X1[c(which(ct.X1=="acinar"),which(ct.X1=="delta"),which(ct.X1=="alpha"),which(ct.X1=="gamma"))]
#data.X4 = cbind(data.X2[,which(ct.X2=="beta")], data.X2[,which(ct.X2=="activated_stellate")],
#                data.X2[,which(ct.X2=="ductal")], data.X2[,which(ct.X2=="endothelial")])
#ct.X4=ct.X2[c(which(ct.X2=="beta"),which(ct.X2=="activated_stellate"),which(ct.X2=="ductal"),which(ct.X2=="endothelial"))]
...
######### UMAP plot
umap.out = umap(data.int.seurat, n_neighbors = 50, metric = "cosine", spread = 5)
...
pdf(file = "Neg1_seurat1.pdf", width=8,height=6)
p1
dev.off()
...
pdf(file = "Neg1_seurat2.pdf", width=7,height=6)
p1
dev.off()
...
meta.data= data.frame(method = c(rep("data1", dim(data.X1)[2]), rep("data2", dim(data.X2)[2])))
row.names(meta.data)=c(colnames(data.X1), colnames(data.X2))
all.data <- CreateSeuratObject(counts = all.RNA, project = "panc8", min.cells = 5, min.features = 50, meta.data = meta.data)
...
data.int.liger = t(cbind(data1.lig,data2.lig))
...
data.plot = data.plot[-(1:8),]
pdf(file = paste0("markers_boxplot_Neg1.pdf"), width=3,height=3)
ggplot(data=data.plot,aes(x=method,y=similarity))+geom_boxplot(outlier.shape = NA)+ylim(0,1.01)+ylab("jaccard similarity")+
...
data.plot = data.frame(similarity=c(sim.gene.det.mat), method=rep(c("SMAI", "fastMNN", "Scanorama","Seurat","LIGER"),each=8))
data.plot = data.plot[-(1:8),]
pdf(file = paste0("FD_boxplot_Neg1.pdf"), width=3,height=3)
ggplot(data=data.plot,aes(x=method,y=similarity))+geom_boxplot(outlier.shape = NA)+ylim(-0.01,1)+ylab("False Positive Rate")+
  geom_point(position = position_jitter(width = 0.2), color = "black") +
  theme(axis.text.x = element_text(angle = 40, vjust = 1, hjust=1)) +
  theme(axis.text=element_text(size=15),
        axis.title=element_text(size=15,face="bold"),
...
data.plot = data.frame(similarity=c(sim.gene.det.mat), method=rep(c("SMAI", "fastMNN", "Scanorama","Seurat","LIGER"),each=8))
data.plot = data.plot[-(1:8),]
pdf(file = paste0("power_boxplot_Neg1.pdf"), width=3,height=3)
...
pdf(file = paste0("Neg1_dist.pdf"), width=3,height=3)
