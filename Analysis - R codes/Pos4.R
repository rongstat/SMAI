...
########################################
data0=readH5AD("small_atac_gene_activity.h5ad", reader = "python")
...
dup.ind=intersect(which(duplicated(data.X1)), which(duplicated(data.X2)))
data.X1=data.X1[-(dup.ind),]
data.X2=data.X2[-(dup.ind),]
...
mnn.out = findMutualNN(t(data1),t(data2), k1=25)
...
all.data <- CreateSeuratObject(counts = all.RNA, project = "atac", min.cells = 3, min.features = 10,
                               meta.data = meta.data)
all.data <- SplitObject(all.data, split.by = "method")
all.data <- lapply(X = all.data, FUN = function(x) {
  x <- FindVariableFeatures(x, selection.method = "vst", nfeatures = 2000)
...
pdf(file = "Pos4_db&k.pdf", width=3.5,height=3.5)
...
highlight_df = data.plot[1,]
pdf(file = "Pos4_ch&sp.pdf",  width=3.5,height=3.5)
...
pdf(file = "Pos4_SMAI1.pdf", width=8,height=6)
p1
dev.off()
...
pdf(file = "Pos4_SMAI2.pdf", width=7,height=6)
p1
dev.off()
...
data.plot = data.frame(similarity=c(sim.gene.det.mat), method=rep(c("SMAI", "fastMNN", "Scanorama","Seurat","LIGER"),each=6))

pdf(file = paste0("markers_boxplot_Pos4.pdf"), width=3,height=3)
