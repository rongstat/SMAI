...
data0=readH5AD("data/global.h5ad",reader = "python")
table(data0@colData$Organ)

counts = data0@assays@data@listData$X
dim(counts)
colnames(counts) = rownames(data0@colData)
rownames(counts) = names(data0@rowRanges)

set.seed(666)
id.lln=sample(sum(data0@colData$Organ=="LLN"),4000)
data.X1 =  as.matrix(counts[,which(data0@colData$Organ=="LLN")[id.lln]])
ct.X1 = data0@colData$Manually_curated_celltype[which(data0@colData$Organ=="LLN")[id.lln]]
table(ct.X1)

id.mln=sample(sum(data0@colData$Organ=="MLN"),4000)
data.X2 =  as.matrix(counts[,which(data0@colData$Organ=="MLN")[id.mln]])
ct.X2 = data0@colData$Manually_curated_celltype[which(data0@colData$Organ=="MLN")[id.mln]]
table(ct.X2)

(sum(is.element(ct.X1,base::intersect(ct.X1,ct.X2)))+sum(is.element(ct.X2,base::intersect(ct.X1,ct.X2))))/(length(ct.X1)+length(ct.X2))
#[1] 0.958
sum(table(ct.X1)*table(ct.X2))/sqrt(sum(table(ct.X1)^2))/sqrt(sum(table(ct.X2)^2))
#[1] 0.96
...
write.table(data.X1.smai, file="pos6_data1.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
write.table(data.X2.smai, file="pos6_data2.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)
...
save(out.smai, data.X1.smai, data.X2.smai, 
     ...
     file="pos6_align.RData")

load("~/Dropbox/Eric-Rong-James/Alignment/RCodes/revision/pos6_align.RData")
################Pamona

#run pamona_run.py in terminal

data1.pamona = t(read.csv("~/Dropbox/Eric-Rong-James/Alignment/RCodes/revision/pamona/pos6_data1.csv", header=F))
data2.pamona = t(read.csv("~/Dropbox/Eric-Rong-James/Alignment/RCodes/revision/pamona/pos6_data2.csv", header=F))
...
data1.scot = t(read.csv("~/Dropbox/Eric-Rong-James/Alignment/RCodes/revision/scot/pos6_data1.csv", header=F))
data2.scot = t(read.csv("~/Dropbox/Eric-Rong-James/Alignment/RCodes/revision/scot/pos6_data2.csv", header=F))
...
data.plot=data.plot[-(8),]
pdf(file = "Figs/Pos6_db&corr_no_scot.pdf",  width=3.5,height=3.5)
...
highlight_df = data.plot[1,]
data.plot=data.plot[-(7:8),]
pdf(file = "Pos6_ch&sp_no_scot_pamona.pdf",  width=3.5,height=3.5)
sp <- ggplot(data.plot, aes(x=ind.bat, y=corr)) +
  geom_point(size=2) + theme(axis.text.x = element_text(angle = 40, vjust = 1, hjust=1)) +
  theme(axis.text=element_text(size=15),
        axis.title=element_text(size=15,face="bold"),
...
pdf(file = "Pos6_SMAI1.pdf", width=13,height=6)
p1
dev.off()
...
pdf(file = "Pos6_SMAI2.pdf", width=7,height=6)
p1
dev.off()
...
k=k+1
(sample1.in=which(ct.X1 == levels(ct.X2)[k]))
(sample2.in=which(ct.X2 == levels(ct.X2)[k]))

if((table(ct.X1)+table(ct.X2))[k]>20){
pval.X1=c()
pval.X2=c()
pval.smai=c()
pval.fmnn=c()
...
for(i in 1:dim(data.X1.smai)[1]){
  ...
  if(length(sample1.in)>1){
    pval.X1[i]=t.test(data.X1.smai[i,sample1.in], data.X1.smai[i,-sample1.in], alternative="greater")$p.value
  }
  if(length(sample2.in)>1){
    pval.X2[i]=t.test(data.X2.smai[i,sample2.in], data.X2.smai[i,-sample2.in], alternative="greater")$p.value
  }
  ...
  pval.scot[i] = t.test(c(data2.scot[i,sample2.in],data1.scot[i,sample1.in]),
                        c(data2.scot[i,-sample2.in],data1.scot[i,-sample1.in]), alternative="greater")$p.value
  
}
pval.liger=cluster.results[which(cluster.results$group==levels(ct.X2)[k]),]$pval
pval.liger=pval.liger[match(rownames(data.X1.smai),cluster.results[which(cluster.results$group==levels(ct.X2)[k]),]$feature)]
...
gene.out[[7]] = which(p.adjust(pval.scot, method="BH")<alpha)

sim.gene.det = c()
for(i in 1:6){
  sim.gene.det[i] = jaccard(gene.out[[1]], gene.out[[i+1]])
}
sim.gene.det.mat = rbind(sim.gene.det, sim.gene.det.mat)
row.names(sim.gene.det.mat)[1] =  levels(ct.X2)[k]}
#repeat the above chunk for various k

data.plot = data.frame(similarity=c(sim.gene.det.mat), method=rep(c("SMAI", "fastMNN", "Scanorama","Seurat","LIGER","SCOT"),each=26))

pdf(file = paste0("markers_boxplot_Pos6_20.pdf"), width=3,height=3)
ggplot(data=data.plot,aes(x=method,y=similarity))+geom_boxplot(outlier.shape = NA)+ylim(-0.01,1)+ylab("jaccard similarity")+
...
########### preserve DE genes for rare cell types

k=0
k=k+1
i=match(names(which(table(factor(ct.X2))/length(ct.X2)<0.005 & (table(ct.X1)+table(ct.X2))>20)),row.names(sim.gene.det.mat))[k]
data.plot = data.frame(similarity=c(sim.gene.det.mat[i,]), method=rep(c("SMAI", "fastMNN", "Scanorama","Seurat","LIGER","SCOT"),each=1))
pdf(file = paste0(row.names(sim.gene.det.mat)[i],"_markers_boxplot_Pos6.pdf"), width=3,height=3)
ggplot()+ylim(0,1)+ylab("jaccard similarity")+
  geom_bar(data=data.plot,aes(x=method,y=similarity),stat="identity") +
  theme(axis.text.x = element_text(angle = 40, vjust = 1, hjust=1)) +
  theme(axis.text=element_text(size=15),
        axis.title=element_text(size=15,face="bold"),
        legend.key.size = unit(1, 'cm'),
