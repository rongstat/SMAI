...
RNA = counts(sce)
spatial=readRDS('PosS1/smFISH_counts.Rds')
spatial.meta=readRDS('PosS1/metadata.Rds')
table(spatial.meta$embryo)
spatial=spatial[,which(spatial.meta$embryo=="embryo2")]
rownames(RNA)=rowData(sce)[,2][match(rownames(RNA),rowData(sce)[,1])]
dim(RNA)
dim(spatial)

RNA.sub = t(RNA[match(rownames(spatial),rownames(RNA)),])
dim(RNA.sub)
...
out1 = matrix(ncol=3, nrow=k)
out2 = matrix(ncol=3, nrow=k)
out3 = matrix(ncol=3, nrow=k)
out4 = matrix(ncol=3, nrow=k)
out5 = matrix(ncol=3, nrow=k)
out6 = matrix(ncol=3, nrow=k)
for(i in gene.sel){
  ...
  data.X2 = as.matrix(all.data$RNA@assays$RNA@data)
   data.anchors <- FindIntegrationAnchors(object.list = all.data, anchor.features = features, k.filter = 50, verbose = FALSE)
  ...
  all.data <- RunPCA(all.data, pc.genes = all.data@var.genes, npcs = 30, verbose = FALSE)
  ...
  set.seed(27)
  out.smai <- align(data.X1, data.X2, denoise="scree", dir.map = "2to1", r.max=33, cutoff = 1.25,
                    sel1=unique(mnn.out$first),  sel2=unique(mnn.out$second),
                    prop.align=0.99*min(c(length(unique(mnn.out$first))/dim(data1)[2],
  ...
  #print(c(i,out1[i,3],out2[i,3],out5[i,3]))
...
boxplot(out1[,1],out2[,1],out3[,1],out4[,1],out5[,1],out6[,1])
boxplot(out1[,2],out2[,2],out3[,2],out4[,2],out5[,2],out6[,2])
boxplot(out1[,3],out2[,3],out3[,3],out4[,3],out5[,3],out6[,3])
...
p <- ggplot(data.plot, aes(x=reorder(method, tau, FUN=median, na.rm=T), y=tau)) +
  geom_boxplot(outlier.shape = NA) + ylab("kendall's tau") + xlab("method")+
  geom_point(position = position_jitter(width = 0.2), color = "black",size=0.2) +
...
pdf(file = "PosS1_kend_top50_cor.pdf", width=3,height=3)
p
dev.off()
...
#SMAI partial alignment test
mnn.out = findMutualNN(t(data1),t(data2), k1=10)
set.seed(3)
out.smai <- align(data.X1, data.X2, dir.map = "2to1", denoise="scree",
                  r.max=33, cutoff = 1.3, cutoff.t=1.8,
                  sel1=unique(mnn.out$first),  sel2=unique(mnn.out$second),
                  prop.align=0.4, t=3)
out.smai$p.value
#[1] 0.9491619
